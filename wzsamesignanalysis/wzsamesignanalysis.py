...
chan = {ch.ee:'ee', ch.mm:'mm', ch.me:'me'}
...
class wzsamesignanalysis(analysis):
  def init(self):
    ...
    if not self.isData:
      ...
      # Pile Up, remember to set it up later again
      self.PUweight = 1#puWeight_5TeV(self.tchain, False)
      # Trigger prefiring
    ...
    for key_chan in chan:
      ...
      for key_level in level:
        ...
        for key_syst in systlabel.keys():
          ...
          self.NewHisto('InvMass', ichan,ilevel,isyst, 20, 0, 100)
          self.NewHisto('DYMass',  ichan,ilevel,isyst, 200, 70, 110)
          self.NewHisto('MaxDeltaPhi',  ichan,ilevel,isyst, 20, 0, 1)
          self.NewHisto('MaxDeltaR'  ,  ichan,ilevel,isyst, 20, 0, 4)
          self.NewHisto('l0_genFlavor',  ichan,ilevel,isyst, 30, -0.5, 29.5)
          self.NewHisto('l1_genFlavor',  ichan,ilevel,isyst, 30, -0.5, 29.5)
  ...
  def FillHistograms(self, leptons, jets, pmet, ich, ilev, isys):
    ...
    self.SetWeight(isys)
    #mz = InvMass(leptons[0],leptons[1])
    l0 = leptons[0]; l1 = leptons[1]

    # Re-calculate the observables
    l0 = TLorentzVector(leptons[0].p)
    l1 = TLorentzVector(leptons[1].p)
    l0pt = l0.Pt(); l0eta = l0.Eta(); l0phi = l0.Phi()
    l1pt = l1.Pt(); l1eta = l1.Eta(); l1phi = l1.Phi()
    
    # Enjoy the lepton high level variables 
    maxdphi = l1.DeltaPhi(l0)    
    deltaR  = l1.DeltaR(l0)
    dipt   = (l0+l1).Pt()
    htmiss  = dipt
    mll   = (l0+l1).M()
    ...
    if njet > 0:
      ...
      htmiss = (l0+l1+TLorentzVector(jet0.P())).Pt()
    if njet > 1:
      jet1 = jets[1]
      j1pt = jet1.Pt(); j1eta = jet1.Eta(); j1phi = jet1.Phi()
      j1csv = jet1.GetCSVv2(); j1deepcsv = jet1.GetDeepCSV()
      htmiss = (l0+l1+TLorentzVector(jet0.P())+TLorentzVector(jet1.P())).Pt()
    else:
    ...
    self.GetHisto("l0_genFlavor",     ich, ilev, isys).Fill(ord(leptons[0].mcmatch), self.weight)
    self.GetHisto("l1_genFlavor",     ich, ilev, isys).Fill(ord(leptons[1].mcmatch), self.weight)
    ...
    self.GetHisto('MaxDeltaPhi',  ich,ilev,isys).Fill(deltaR, self.weight)
  ...
  def insideLoop(self, t):
    ...
    ### And look for SS pair and classify the channel
    l0 = self.selLeptons[0]
    l1 = self.selLeptons[1]
    if l0.charge * l1.charge < 0: return False

    totId = abs(l0.GetPDGid()) + abs(l1.GetPDGid()) 
    ich = -1
    if   totId == 22: ich = ch.ee
    elif totId == 24: ich = ch.me
    elif totId == 26: ich = ch.mm
    tleps = [l0, l1]
    zleps = [l0, l1]
    ...
    ###########################################
    trigger = {
     ch.ee:t.HLT_HIEle20_WPLoose_Gsf or t.HLT_HIEle20_Ele12_CaloIdL_TrackIdL_IsoVL_DZ,
     ch.me:t.HLT_HIL3Mu20 or t.HLT_HIEle20_WPLoose_Gsf,
     ch.mm:t.HLT_HIL3Mu20 or t.HLT_HIL3DoubleMu10, #  t.HLT_HIL3DoubleMu0
    }
    ...
    #    if   ich == ch.mee: passTrig = trigger[ch.mm] and not trigger[ch.ee]
    #    elif ich == ch.emm: passTrig = trigger[ch.mm]
    #    elif ich == ch.mm: passTrig = trigger[ch.mm]
    #    else:               passTrig = False
    ...
    if not self.isData:
      for lep in self.selLeptons:
        if lep.IsMuon():
          sf, err = self.GetSFandErr('MuonIsoSF, MuonIdSF', max(20,lep.Pt()), TMath.Abs(lep.Eta()))
          self.SFmuon*=sf
          ...
          ssf, serr = self.GetSFfromTGraph(elecsf,max(20,lep.Pt()))
          rsf, rerr = self.GetSFfromTGraph(recosf,max(20,lep.Pt()))
          sf = (1./ssf)*rsf;
    ...
    if nJets > 1:
      htmiss = (l1.P()+l0.P()+TLorentzVector(self.selJets[0].P())+TLorentzVector(self.selJets[1].P())).Pt()
    elif nJets > 0:
      htmiss = (l0.P()+l1.P()+TLorentzVector(self.selJets[0].P())).Pt()
    else:
      htmiss = (l0.P()+l1.P()).Pt()
    ...
    if not self.isData:
      self.PUSF   = 1#t.puWeight
      self.PUUpSF = 1#t.puWeightUp
      self.PUDoSF = 1#t.puWeightDown
    else:
    ...
    for isyst in systlabel.keys():
      ...
      #if tleps[0].passTightID and tleps[1].passTightID and pmet.Pt() > 25:
      #  self.FillAll(ich, lev.srtight, isyst, leps, jets, pmet)
      if tleps[0].passTightID and tleps[1].passTightID:
